...
/** Build RBush spatial index from pub points */
function buildIndex(pubFeatures) {
  pubIndex = turfRbush();
  pubIndex.load(pubFeatures);
}
...
function getSpoke(pubs) {
  ...
  const nearestpubs = turf.featureCollection(
    pubIndex.knn(centerPoint, 5), // ⚡ fast nearest search
  );

  const lines = turf.featureCollection(
    nearestpubs.features.map((pub) => {
      return turf.lineString([centerPoint.geometry.coordinates, pub.geometry.coordinates]);
    }),
  );
...
function addLayers(pubs, nearest, route) {
  ...
  map.addLayer({
    ...
    source: "points",
    paint: {
      "circle-radius": ["interpolate", ["linear"], ["zoom"], 0, 2, 10, 8],
      "circle-opacity": 1,
      "circle-blur": 0,
      "circle-color": "#a7a79b",
    },
  ...
  map.addLayer({
    ...
    source: "newPoint",
    paint: {
      "circle-radius": ["interpolate", ["linear"], ["zoom"], 0, 3, 10, 10],
      "circle-opacity": 1,
      "circle-blur": 0,
      "circle-color": "#035690",
    },
